/*10. [Parking con la clase Instante]. Recupere la soluci—n del ejercicio 46 de la relaci—n de problemas III (Parking con clases y struct)
 Re-escr’balo para que los instantes sean objetos de la clase Instante. Por lo tanto, tiene que eliminar la funci—n que calculaba los minutos entre dos instantes y hacerlo dentro de la clase Instante.
 Finalidad: MŽtodos que reciben como par‡metro un objeto de la misma clase. Dificul- tad Baja.
*/
...
using namespace std; 
 ...
 class Instante{
 private:
     static const int paso_medida = 60;
     int hora;
     int minuto;
     int segundo;
 public:
     Instante(int h, int m, int s){
         hora = h;
         minuto = m;
         segundo = s ;
     }
